...
const zeroPops = allDistricts.filter(d => d.population === 0);
console.log(`Districts with pop > 0: ${allDistricts.length - zeroPops.length}`);
console.log(`Districts with pop == 0: ${zeroPops.length}`);
if (zeroPops.length > 0) {
  console.log("Sample 10 zero pop districts:", zeroPops.slice(0, 10).map(d => ({ state: d.state, district: d.district, popRaw: d.popRaw })));
}
